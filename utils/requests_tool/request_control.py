...
from utils.requests_tool.encryption_algorithm_control import encryption     # 导入加密方式
...
class RequestControl:
    ...
    def multipart_in_headers(
        ...
        header = ast.literal_eval(cache_regular(str(header)))               # 在这里将cookies加入请求头
    ...
    def _check_params(
        ...
        _data = {                           # 带着响应内容和一些yaml_data数据解包返回
    ...
    @log_decorator(True)        # 做http请求前加装饰器
    @execution_duration(3000)   # 做http请求时加装饰器，超时时间
    #@encryption("md5")     # 接口是否加密的开关
    def http_request(
            self,
            dependent_switch=True,  # 是否开启依赖用例开关，为true时一定会做检查
            **kwargs
    ):
        """
        请求封装
        ...
        requests_type_mapping = {                                   # 定义了一个字典 requests_type_mapping，将请求类型（如 JSON、PARAMS 等）映射到对应的处理方法
        ...
        # 判断用例是否执行
        if is_run is True or is_run is None:        # 如果 is_run 为 True 或 None，则继续执行后续逻辑；否则跳过该用例
            # 处理多业务逻辑
            ...
            res = requests_type_mapping.get(self.__yaml_case.requestType)(  # 拿到请求数据
                headers=self.__yaml_case.headers,
                method=self.__yaml_case.method,
                **kwargs
            )   

            if self.__yaml_case.sleep is not None:      # 是否休眠
                time.sleep(self.__yaml_case.sleep)

            _res_data = self._check_params(             # 对响应数据进行校验
                res=res,
                yaml_data=self.__yaml_case)

            self.api_allure_step(       # 将执行信息写入allure报告中
                url=_res_data.url,
