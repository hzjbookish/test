...
# -*- coding: utf-8 -*-
'''
    allure报告生成相关，统计测试执行情况
'''
import json
from typing import List, Text
from common.setting import ensure_path_sep
from utils.read_files_tools.get_all_files_path import get_all_files # 导入获取所有文件的方法，可支持yaml文件过滤
from utils.other_tools.models import TestMetrics    # 导入用例执行情况统计的数据
...
class AllureFileClean:
    ...
    def get_failed_case(self) -> List:
        ...
        for i in self.get_testcases():
            if i['status'] == 'failed' or i['status'] == 'broken':  # 检查其状态是否为 failed 或 broken。如果是，则将用例标题和代码路径添加到 error_case 列表中。
