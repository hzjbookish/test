...
def regular(target):
    ...
    try:
        regular_pattern = r'\${{(.*?)}}'    
        while re.findall(regular_pattern, target):                  # 使用 re.findall 查找所有匹配的占位符。如果找到匹配项，则进入循环，逐个替换占位符
            key = re.search(regular_pattern, target).group(1)       # 使用 re.search 查找第一个匹配的占位符，并获取占位符的值
            value_types = ['int:', 'bool:', 'list:', 'dict:', 'tuple:', 'float:']
            if any(i in key for i in value_types) is True:
                func_name = key.split(":")[1].split("(")[0]
                value_name = key.split(":")[1].split("(")[1][:-1]
                if value_name == "":
                    value_data = getattr(Context(), func_name)()    # 用反射的方式调用函数
                else:
                ...
                func_name = key.split("(")[0]   # 获取函数名
                value_name = key.split("(")[1][:-1] # 获取函数参数
                if value_name == "":    # 如果函数无参，则直接调用方法,如 Context().hsot()
                    value_data = getattr(Context(), func_name)()
                else:
                    value_data = getattr(Context(), func_name)(*value_name.split(","))
                target = re.sub(regular_pattern, str(value_data), target, 1)    # 使用 re.sub 将占位符替换为实际的函数调用结果, 1 表示每次只替换第一个匹配项
        return target
