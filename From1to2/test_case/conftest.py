...
# -*- coding: utf-8 -*-
'''
    __init__.py执行完之后执行该文件，该文件存放pytest的一些夹具，不仅可以拿到cookies,还可以调整用例的运行顺序
'''
...
@pytest.fixture(scope="session", autouse=False) # 整个会话开始前执行，清理报告文件
def clear_report():
    """如clean命名无法删除报告，这里手动删除"""
    del_file(ensure_path_sep("\\report"))


@pytest.fixture(scope="session", autouse=True)  # 登录，每个会话执行一次
def work_login_init():
    """
    获取登录的cookie
    :return:
    ...
    data = {
        "username": 13336999328,
        "password": 123456
    }
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    # 请求登录接口

    res = requests.post(url=url, data=data, verify=False, headers=headers)      # verify=False 忽略证书校验
    ...
    for k, v in response_cookie.items():
        ...
        cookies += _cookie  # 将cookies拼接在一起
        # 将登录接口中的cookie写入缓存中，其中login_cookie是缓存名称
    CacheHandler.update_cache(cache_name='login_cookie', value=cookies)


def pytest_collection_modifyitems(items):   # 钩子函数，在测试用例收集完成后立即执行。此时，Fixture 尚未初始化，因此不会涉及 work_login_init() 的执行。
    """
...
@pytest.fixture(scope="function", autouse=True)     # 每个函数执行前执行，用于处理跳过用例，如果不跳过的话，那么就开始进入用例内部了
...
def pytest_terminal_summary(terminalreporter):      # 钩子函数，测试用例执行结束后，生成测试结果报告。
    """
    收集测试结果
    """
